...
public class MongoDbRestore extends MongoUtil{
	...
	public MongoDbRestore(String inputput,String host,String database){
		INPUT_DIR = inputput;
		DATABASE_HOST = host;
			...
			if(files != null){
				...
				if(DROP_EXISTING){
					try{
						...
						DB db = MongoDBConnectionManager.getConnection(DATABASE_HOST, "local", DATABASE_USER_NAME, DATABASE_PASSWORD);
	...
	protected void write(CollectionInfo info, DBObject dbo) throws Exception {
		DB db = MongoDBConnectionManager.getConnection(DATABASE_HOST, DATABASE_NAME, DATABASE_USER_NAME, DATABASE_PASSWORD);
	...
	private Collection<String> getCollectionNamesFromFiles(String inputDir) {
		...
		for(File file : files){
			if(file.getName().contains(".bson")){
				...
				int lastIndexOf = collectionName.lastIndexOf('.');
				if(lastIndexOf > 0){
					collectionName = collectionName.substring(0,lastIndexOf);
				}
